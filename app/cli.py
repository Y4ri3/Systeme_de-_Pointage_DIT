...
def register_cli(app):
    ...
    def seed_db():
        """Peuple la base avec des données de test minimales (filières, promotions, salles, comptes, cours)."""
        ...
        for niveau, filiere in promotions_def:
            promo = Promotion.query.filter_by(
                niveau=niveau, filiere_id=filiere.id, annee_academique=annee).first()
            if promo is None:
        ...
        cours_def = [
            (matieres['BD-101'], utilisateurs['professeur'], salles['A101'],
             promotions[('L1', 'Big Data')],
             date.today() + timedelta(days=1), time(9, 0), time(11, 0)),
            (matieres['IA-101'], utilisateurs['professeur'], salles['Amphi B'],
             promotions[('M1', 'Intelligence Artificielle')],
             date.today() + timedelta(days=2), time(14, 0), time(16, 0)),
        ]
        for matiere, professeur, salle, promotion, jour, debut, fin in cours_def:
            existe = Cours.query.filter_by(
                matiere_id=matiere.id, promotion_id=promotion.id, date=jour).first()
            if existe is None:
                cours = Cours(
                    matiere=matiere,
