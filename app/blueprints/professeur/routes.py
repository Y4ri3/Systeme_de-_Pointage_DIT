...
def cancel_course(cours_id):
    ...
    notifications = [
        Notification(
            ...
            cours_id=cours.id,
            type='cours_annule',
            message=f'Le cours de {cours.matiere.nom} du {cours.date.isoformat()} a ete annule. Motif: {motif}',
        )
        for etudiant in cours.promotion.etudiants
    ]
    if notifications:
...
def reschedule_course(cours_id):
    ...
    if not nouvelle_date or not nouvelle_heure_debut or not nouvelle_heure_fin:
        return jsonify({
            'error': 'bad_request',
            'message': 'Les champs date/new_date, heure_debut/new_start_time et heure_fin/new_end_time sont requis.',
            'details': {
                'accepted_fields': {
                    'date': ['date', 'new_date'],
...
def regularize_attendance(cours_id):
    ...
    pointage = Pointage.query.filter_by(
        ...
        Pointage.statut.in_(list(statuts_autorises))
    ).order_by(Pointage.timestamp_pointage.desc()).first()
    ...
    pointage.wifi_detecte = False
    pointage.qr_valide = False
