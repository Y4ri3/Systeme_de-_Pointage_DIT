...
from app.utils.decorators import network_restricted, role_required
...
@etudiant_bp.route("/attendance/checkin-qr", methods=["POST"])
@network_restricted
@role_required("etudiant")
def checkin_qr():
    """Pointage alternatif au visage : scan, depuis le téléphone de l'étudiant, du QR
    affiché en salle pour le cours en cours. Le cours est déduit du token, pas du payload.

    Restreint au réseau Wi-Fi dédié de l'établissement (KIOSK_ALLOWED_NETWORKS) : le
    QR affiché à l'écran suffit à identifier le cours, mais seul le réseau prouve
    que l'étudiant est physiquement sur place au moment du scan.
    """
    data = request.get_json(silent=True) or {}
...
def absences_summary():
    suivis = (
        SuiviAbsences.query.filter_by(etudiant_id=int(get_jwt_identity()))
        .order_by(SuiviAbsences.updated_at.desc())
        .all()
    )
