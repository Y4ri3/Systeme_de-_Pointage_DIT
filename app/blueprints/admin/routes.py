...
def _student_attendance_query(etudiant_id):
    return Pointage.query.filter_by(etudiant_id=etudiant_id).order_by(Pointage.timestamp_pointage.desc())
...
def _student_attendance_summary_payload(etudiant):
    ...
    for pointage in pointages:
        ...
        subject_row = by_subject.setdefault(
            matiere.id,
            {
...
def _parse_id_list(payload, key):
    """Extrait une liste d'ids entiers depuis le payload : liste JSON, valeur
    unique, valeurs repetees (form-data) ou chaine separee par des virgules.
    Retourne None si la cle est absente. Leve ValueError si un id n'est pas
    un entier valide.
    """
...
def _resolve_related_objects(model_cls, ids, label):
    """Resout une liste d'ids vers des instances du modele donne.
    Retourne (objets, None) ou (None, (body, status)) si un id est invalide."""
    objets = []
    for object_id in ids:
        objet = db.session.get(model_cls, object_id)
        if objet is None:
            return None, (
                {
                    "error": "bad_request",
                    "message": f"{label} {object_id} est invalide.",
                    "details": {},
                },
                400,
            )
...
def _create_managed_user(role):
    ...
    if role == "professeur":
        try:
            matiere_ids = _parse_id_list(payload, "matiere_ids") or []
            promotion_ids = _parse_id_list(payload, "promotion_ids") or []
        except ValueError:
            return None, (
                {
                    "error": "bad_request",
                    "message": "matiere_ids et promotion_ids doivent etre des listes d'entiers.",
                    "details": {},
                },
                400,
            )

        matieres_enseignees, error = _resolve_related_objects(Matiere, matiere_ids, "La matiere")
        if error is not None:
            return None, error

        promotions_en_charge, error = _resolve_related_objects(Promotion, promotion_ids, "La promotion")
        if error is not None:
            return None, error

    try:
        photo_path = _save_user_photo(role, required=False)
    ...
    if role == "professeur":
        user.matieres_enseignees = matieres_enseignees
        user.promotions_en_charge = promotions_en_charge
...
def _update_managed_user(user):
    ...
    if user.role == "professeur":
        try:
            matiere_ids = _parse_id_list(payload, "matiere_ids")
            promotion_ids = _parse_id_list(payload, "promotion_ids")
        except ValueError:
            return (
                {
                    "error": "bad_request",
        ...
        if matiere_ids is not None:
            matieres, error = _resolve_related_objects(Matiere, matiere_ids, "La matiere")
            if error is not None:
                return error
            user.matieres_enseignees = matieres

        if promotion_ids is not None:
            promotions, error = _resolve_related_objects(Promotion, promotion_ids, "La promotion")
            if error is not None:
                return error
            user.promotions_en_charge = promotions
...
def get_student(etudiant_id):
    ...
    suivis = SuiviAbsences.query.filter_by(etudiant_id=etudiant.id).all()
    pointages = (
        Pointage.query.filter_by(etudiant_id=etudiant.id)
        .order_by(Pointage.timestamp_pointage.desc())
        .limit(20)
        .all()
    )
...
def get_professor(professeur_id):
    ...
    cours = (
        Cours.query.filter_by(professeur_id=professeur.id)
        .order_by(
            Cours.date.desc(),
...
def create_course():
    ...
    if matiere not in professeur.matieres_enseignees:
        return (
            jsonify(
                {
                    "error": "professeur_matiere_non_associee",
                    "message": "Ce professeur n'est pas rattaché à cette matière.",
                    "details": {},
                }
            ),
            400,
    ...
    if promotion not in professeur.promotions_en_charge:
        return (
            jsonify(
                {
                    "error": "professeur_promotion_non_associee",
                    "message": "Ce professeur n'est pas rattaché à cette promotion.",
                    "details": {},
                }
            ),
            400,
    ...
    conflicts = detect_schedule_conflicts(
        date_cours, heure_debut, heure_fin, professeur.id, salle.id, promotion.id
    )
    if conflicts:
        return (
            jsonify(
...
def update_matiere(matiere_id):
    ...
    return (
        jsonify({"message": "Matiere mise a jour avec succes.", "subject": serialize_matiere(matiere)}),
        200,
    )
...
def update_filiere(filiere_id):
    ...
    return (
        jsonify({"message": "Filiere mise a jour avec succes.", "department": serialize_filiere(filiere)}),
        200,
    )
...
def create_promotion():
    ...
    return (
        jsonify({"message": "Promotion creee avec succes.", "promotion": serialize_promotion(promotion)}),
        201,
    )
...
def update_promotion(promotion_id):
    ...
    return (
        jsonify(
            {"message": "Promotion mise a jour avec succes.", "promotion": serialize_promotion(promotion)}
        ),
        200,
    )
