...
from app.services import attendance_events, face_service
...
def enregistrer_pointage(etudiant_id, cours_id, selfie_bytes, selfie_filename,
    ...
    if timestamp <= limite_present:
        ...
        raise PointageError(
            'hors_delai', 'Le délai de pointage pour ce cours est dépassé.', 400)

    deja_pointe = Pointage.query.filter_by(
        etudiant_id=etudiant_id, cours_id=cours_id
    ).filter(Pointage.statut.in_(['present', 'retard'])).first()
    if deja_pointe is not None:
        raise PointageError(
            'pointage_deja_enregistre', 'Un pointage existe déjà pour ce cours.', 409)
...
def identifier_etudiant_pour_borne(selfie_bytes, selfie_filename, timestamp=None):
    ...
    try:
        ...
        liveness_ok = (
            liveness_info.get('is_real_face') and
            liveness_probability >= current_app.config['ARSA_FACE_LIVENESS_THRESHOLD']
        )
        if not liveness_ok:
            raise PointageError(
                'visage_non_vivant',
...
def _build_attendance_context(etudiant, timestamp):
    ...
    if active_course is not None:
        already_checked_in = Pointage.query.filter_by(
            cours_id=active_course.id,
            etudiant_id=etudiant.id,
        ).filter(Pointage.statut.in_(['present', 'retard'])).first() is not None
...
def _build_face_verification(match_info, liveness_info):
    ...
    similarity_ok = similarity_score is None or similarity_score >= current_app.config['ARSA_FACE_MATCH_THRESHOLD']
    liveness_probability = liveness_info.get('liveness_probability') or 0
    liveness_ok = (
        liveness_info.get('is_real_face') and
        liveness_probability >= current_app.config['ARSA_FACE_LIVENESS_THRESHOLD']
...
def _build_face_justificatif(verification):
    ...
    return f"face_match={verification.get('match')};similarity_score={similarity};liveness_probability={liveness}"
