...
def resoudre_qr_token(token):
    """Décode un token QR de cours ; retourne le cours_id, ou None si invalide/expiré."""
    serializer = _get_serializer()
    try:
    ...
    return data.get("cours_id") if isinstance(data, dict) else None
