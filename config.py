...
class Config:
    ...
    KIOSK_ALLOWED_NETWORKS = [
        network.strip()
        for network in (os.environ.get("KIOSK_ALLOWED_NETWORKS") or "").split(",")
        if network.strip()
    ]
    ...
    CORS_ALLOWED_ORIGINS = [
        origin.strip()
        for origin in (
            os.environ.get("CORS_ALLOWED_ORIGINS") or "http://localhost:5177,http://127.0.0.1:5177"
        ).split(",")
        if origin.strip()
    ]
...
class ProductionConfig(Config):
    ...
    def __init__(self):
        required = ("SECRET_KEY", "JWT_SECRET_KEY", "DATABASE_URL", "KIOSK_ALLOWED_NETWORKS", "REDIS_URL")
        if missing := [name for name in required if not os.environ.get(name)]:
            raise RuntimeError(
                "Variables d'environnement obligatoires manquantes en production: " + ", ".join(missing)
            )
