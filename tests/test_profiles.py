...
def test_listes_principales_supportent_la_pagination(app):
    ...
    courses_resp = client.get('/api/v1/professeur/courses?page=1&per_page=1', headers={
        'Authorization': f'Bearer {_login(client, data["professeur"].email)}'
    })
    assert courses_resp.status_code == 200
    courses_payload = courses_resp.get_json()
    ...
    etu_courses_resp = client.get('/api/v1/etudiant/courses?page=1&per_page=1', headers={
        'Authorization': f'Bearer {_login(client, data["etudiant_1"].email)}'
    })
...
def test_responsable_peut_modifier_un_professeur_et_regenerer_son_mot_de_passe_temporaire(app):
    ...
    failed_login_resp = client.post('/api/v1/auth/login', json={
        'email': 'reset.prof@test.com',
