...
def test_reset_password_change_le_mot_de_passe_et_permet_la_connexion(app):
    ...
    ancien_login = client.post('/api/v1/auth/login', json={
        'email': user.email, 'password': 'AncienMdp123!',
    })
    assert ancien_login.status_code == 401

    nouveau_login = client.post('/api/v1/auth/login', json={
        'email': user.email, 'password': 'NouveauMdp456!',
    })
...
def test_reset_password_token_est_a_usage_unique(app):
    ...
    premier = client.post('/api/v1/auth/reset-password', json={
        'token': token,
